<template>
  <div class="wraps">
    ...
    <el-table border :data="tableData" v-loading="loading" style="width: 100%;margin-top: 30px;">
      <el-table-column prop="name" label="名字" />
      <el-table-column prop="desc" label="描述" />
      <el-table-column prop="id" label="id" />
      <el-table-column prop="tags" label="关联的tags">
        <template #default="scope">
          <el-tag v-for="(item, index) in scope.row.tags" :key="index" style="margin-right:10px;">{{ item.name
            }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column>
...
import { addUser, updateUser, delUser, getList, addTags } from '@/api/curd'
